...
'weights_l' :  tf.Variable(tf.random_normal([512, n_classes ])),
...
# Construct model
logits = cnn.cnn_model(input_layer = x, dropout = keep_prob, param = model_parameters)
...
with tf.Session() as sess:
    ...
    while step <= training_iters:
        ...
        if step % display_step == 0:
            ...
            acc = sess.run(accuracy, feed_dict={x: random_images, y: random_labels, keep_prob: dropout}) #keep_prob zmienione z 1. na dropout
    ...
    # Calculate accuracy 
    print("Testing Accuracy:", sess.run(accuracy, feed_dict={x: test_X, y: test_Y, keep_prob: dropout}))